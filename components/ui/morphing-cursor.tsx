...
export function MagneticImage({ 
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (innerImageRef.current) {
        innerImageRef.current.style.transform = `translate(${-currentPos.current.x}px, ${-currentPos.current.y}px)`
      }
  ...
  return (
    <div
      ...
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
      className={cn("relative cursor-none select-none", className)}
    >
      {/* Base image layer */}
      <img
        src={baseImage}
      ...
      {/* Circular reveal of hover image */}
      <div
        ref={circleRef}
